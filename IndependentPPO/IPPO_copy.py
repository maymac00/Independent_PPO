...
class IPPO:
    ...
    def __init__(self, args, env, run_name=None):
        ...
        if run_name is not None:
            ...
            self.run_name = f"{self.env}__{self.tag}__{self.seed}__{int(time.time())}__{np.random.randint(0, 100)}"
    ...
    def train(self, load_from_checkpoint=None, init_global_step=0):
        ...
        IPPO.summary_w, self.wandb_path = init_loggers(self.save_dir, self.run_name, self.init_args)

        # Init actor-critic setup
        self.actor, self.critic, self.a_optim, self.c_optim, self.buffer = {}, {}, {}, {}, {}
        self.past_actions_memory = {}
        # TODO: Reset intial memory for the previous actions taken
        # initial_memory = [ACTIONS.index(CommonsGame.STAY) / len(ACTIONS) for i in range(self.past_actions_memory)]

        if load_from_checkpoint is not None:
            self._load_models_from_files(load_from_checkpoint)
            for k in self.agents:
                # self.past_actions_memory[k] = deque(initial_memory, maxlen=self.past_actions_memory)
        ...
        self.metrics = {
            ...
            'ep_count': init_global_step / self.max_steps,
            'start_time': time.time(),
            'reward_q': [],
            'reward_per_agent': [],
        ...
        for update in range(1, self.n_updates + 1):
            if self.anneal_lr:
                ...
                if self.tb_log: IPPO.summary_w.add_scalar('Training/Actor LR', a_opt.param_groups[0]["lr"],
                                                               self.metrics["global_step"])
                if self.tb_log: IPPO.summary_w.add_scalar('Training/Critic LR', c_opt.param_groups[0]["lr"],
            ...
            if not self.parallelize:
                ...
                self.metrics['global_step'] += self.n_envs * self.max_steps
            ...
            if self.tb_log: self.summary_w.add_scalar('Training/SPS', sps, self.metrics["global_step"])
    ...
    def _update(self):
        ...
        for k in self.agents:
            ...
            for epoch in range(self.n_epochs):
                ...
                if self.tb_log: IPPO.summary_w.add_scalar('Agent_' + str(k) + "/" + 'Actor non-clipped loss', actor_loss.mean(),
                                                               (self.metrics[
                                                                    "global_step"] / self.max_steps) * self.n_epochs + epoch)
                ...
                if self.tb_log: IPPO.summary_w.add_scalar('Agent_' + str(k) + "/" + 'Actor loss', actor_loss,
                                                               (self.metrics[
                                                                    "global_step"] / self.max_steps) * self.n_epochs + epoch)


                if self.tb_log: IPPO.summary_w.add_scalar('Agent_' + str(k) + "/" + 'Entropy', self.ent_coef * entropy_loss,
                                                               (self.metrics[
                                                                    "global_step"] / self.max_steps) * self.n_epochs + epoch)
            ...
            for epoch in range(self.n_epochs * self.critic_times):
                ...
                if self.tb_log: IPPO.summary_w.add_scalar('Agent_' + str(k) + "/" + 'Critic loss', critic_loss,
                                                               (self.metrics[
                                                                    "global_step"] / self.max_steps) * self.n_epochs * self.critic_times + epoch)
    ...
    def _sim(self, render=False, masked=False, pause=0.01):
        ...
        for step in range(self.n_steps):
            ...
            if self.metrics["global_step"] % self.max_steps == 0:
